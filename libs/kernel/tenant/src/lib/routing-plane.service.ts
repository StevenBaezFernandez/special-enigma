...
export class RoutingPlaneService {
  ...
  async enforceProxyRoute(input: {
    ...
    const snapshot = await this.em.findOne(TenantRoutingSnapshot, { tenantId: input.tenantId });

    if (!snapshot || !this.verifySnapshot(snapshot) || this.isExpired(snapshot)) {
      throw new ForbiddenException('No valid routing snapshot available for proxy enforcement');
    }

    const serviceTarget = snapshot.routeTargets?.serviceEndpoints?.[input.service];
    if (!serviceTarget?.url) {
      throw new ForbiddenException(`Service ${input.service} is not allowed by routing snapshot`);
    }

    if (input.generation && input.generation !== snapshot.generation) {
      throw new ConflictException(`Routing generation fence violation for tenant ${input.tenantId}`);
    }

    if (input.signature && input.signature !== snapshot.signature) {
      throw new ForbiddenException(`Routing signature mismatch for tenant ${input.tenantId}`);
    }

    if (serviceTarget.url !== input.requestedTarget) {
      throw new ForbiddenException(`Requested route target deviates from signed routing snapshot`);
    }

    return { target: serviceTarget.url, generation: snapshot.generation, signature: snapshot.signature };
  }
  ...
  private async evaluateRoutingPolicies(tenantId: string): Promise<any> {
    ...
    if (capacity.load > 0.85) {
      this.logger.warn(`Region ${primary} is at high capacity (${(capacity.load * 100).toFixed(2)}%).`);
    }
    ...
    await this.createSnapshot(tenantId, targets, { expectedGeneration: undefined });
    return targets;
  }
  ...
  async createSnapshot(
    ...
    if (typeof options?.expectedGeneration === 'number' && (existing?.generation ?? 0) !== options.expectedGeneration) {
  ...
  private async resolveServiceEndpoints(input: {
  ...
  }): Promise<Record<string, { url: string; version: string; source: string }>> {
    const fromControlPlane = await this.getVersionedEndpointsFromControlPlane(input);
    if (fromControlPlane) {
    ...
    if (catalog && typeof catalog === 'object') {
      ...
      const regionCatalog = modeCatalog?.[input.primaryRegion] || modeCatalog?.default;
  ...
  private async getVersionedEndpointsFromControlPlane(input: {
    ...
    mode: TenantMode;
    primaryRegion: string;
  }): Promise<Record<string, { url: string; version: string; source: string }> | null> {
    try {
      ...
      return rows.reduce((acc: Record<string, { url: string; version: string; source: string }>, row: any) => {
        acc[row.service] = {
          url: row.endpoint,
  ...
  private async getRegionalCapacity(region: string): Promise<RegionalCapacity> {
    const fromPrometheus = await this.getRegionalCapacityFromPrometheusMirror(region);
    if (fromPrometheus) {
      return fromPrometheus;
    }

    const fromControlPlane = await this.getRegionalCapacityFromControlPlane(region);
    if (fromControlPlane) {
      return fromControlPlane;
    }
  ...
  private async getRegionalCapacityFromPrometheusMirror(region: string): Promise<RegionalCapacity | null> {
    try {
      ...
      return { load: parseFloat(result[0].load_factor), observedAt, source: 'prometheus' };
    } catch (err) {
      this.logger.warn(
  ...
  private async getRegionalCapacityFromControlPlane(region: string): Promise<RegionalCapacity | null> {
    ...
    try {
      const response = await axios.get(`${baseUrl}/v1/routing/capacity/${region}`, { timeout: 3000 });
      const payload = response.data as { loadFactor: number; observedAt: string };
