...
export class MxFiscalDocumentBuilder implements FiscalDocumentBuilder {
  ...
  async build(invoice: Invoice, tenantConfig: TenantFiscalConfig, customer: CustomerBillingInfo): Promise<string> {
    ...
    // Assuming XSLT path relative to root or copied to dist.
    // Using fixed path as per original service.
    ...
    if (privateKey) {
        ...
        // Throw or allow unsigned for testing if key missing?
        // Original threw error implicitly or explicitly.
